...
bash -l -c "source $HOME/.bashrc; export PATH=$HOME/bin:$PATH; hash -r; which code >> $HOME/vscode-env-debug.log; $HOME/bin/code --version" | head -n 1 > "$HOME/vscode-verify.log" 2>&1 || {
...
bash -l -c "source $HOME/.bashrc; which asdf" >> "$HOME/asdf-env-debug.log" 2>&1

# Verify asdf installation
echo "Verifying asdf installation..."
bash -l -c "source $HOME/.bashrc; export PATH=$HOME/bin:$PATH; hash -r; $HOME/bin/asdf --version" > "$HOME/asdf-verify.log" 2>&1 || {
...
bash -l -c "source $HOME/.bashrc; export PATH=$HOME/bin:$PATH; hash -r; $HOME/bin/asdf plugin add nodejs https://github.com/asdf-vm/asdf-nodejs.git || true" > "$HOME/asdf-nodejs-plugin.log" 2>&1 || {
  echo "Error: Failed to add nodejs plugin. See $HOME/asdf-nodejs-plugin.log"
  cat "$HOME/asdf-nodejs-plugin.log"
  exit 1;
}
bash -l -c "source $HOME/.bashrc; export PATH=$HOME/bin:$PATH; hash -r; $HOME/bin/asdf install nodejs 20.18.1" > "$HOME/asdf-nodejs-install.log" 2>&1 || {
  echo "Error: Failed to install Node.js 20.18.1. See $HOME/asdf-nodejs-install.log"
  cat "$HOME/asdf-nodejs-install.log"
  exit 1;
}
bash -l -c "source $HOME/.bashrc; export PATH=$HOME/bin:$PATH; hash -r; $HOME/bin/asdf set -u nodejs 20.18.1" > "$HOME/asdf-nodejs-global.log" 2>&1 || {
  echo "Error: Failed to set Node.js global version. See $HOME/asdf-nodejs-global.log"
  cat "$HOME/asdf-nodejs-global.log"
  exit 1;
}
bash -l -c "source $HOME/.bashrc; export PATH=$HOME/bin:$PATH; hash -r; $HOME/bin/asdf reshim nodejs" > "$HOME/asdf-nodejs-reshim.log" 2>&1 || {
...
bash -l -c "source $HOME/.bashrc; export PATH=$HOME/bin:$PATH; hash -r; node --version" > "$HOME/node-verify.log" 2>&1 || {
  echo "Error: Node.js verification failed. See $HOME/node-verify.log"
  cat "$HOME/node-verify.log"
  exit 1;
}
echo "npm version:"
bash -l -c "source $HOME/.bashrc; export PATH=$HOME/bin:$PATH; hash -r; npm --version" > "$HOME/npm-verify.log" 2>&1 || {
...
bash -l -c "source $HOME/.bashrc; export PATH=$HOME/bin:$PATH; hash -r; $HOME/bin/asdf plugin add pnpm https://github.com/jonathanmorley/asdf-pnpm.git || true" > "$HOME/asdf-pnpm-plugin.log" 2>&1 || {
  echo "Error: Failed to add pnpm plugin. See $HOME/asdf-pnpm-plugin.log"
  cat "$HOME/asdf-pnpm-plugin.log"
  exit 1;
}
bash -l -c "source $HOME/.bashrc; export PATH=$HOME/bin:$PATH; hash -r; $HOME/bin/asdf install pnpm 10.11.0" > "$HOME/asdf-pnpm-install.log" 2>&1 || {
  echo "Error: Failed to install pnpm 10.11.0. See $HOME/asdf-pnpm-install.log"
  cat "$HOME/asdf-pnpm-install.log"
  exit 1;
}
bash -l -c "source $HOME/.bashrc; export PATH=$HOME/bin:$PATH; hash -r; $HOME/bin/asdf set -u pnpm 10.11.0" > "$HOME/asdf-pnpm-global.log" 2>&1 || {
  echo "Error: Failed to set pnpm global version. See $HOME/asdf-pnpm-global.log"
  cat "$HOME/asdf-pnpm-global.log"
  exit 1;
}
bash -l -c "source $HOME/.bashrc; export PATH=$HOME/bin:$PATH; hash -r; $HOME/bin/asdf reshim pnpm" > "$HOME/asdf-pnpm-reshim.log" 2>&1 || {
...
bash -l -c "source $HOME/.bashrc; export PATH=$HOME/bin:$PATH; hash -r; pnpm setup" > "$HOME/pnpm-setup.log" 2>&1 || {
...
# Verify pnpm
echo "pnpm version:"
bash -l -c "source $HOME/.bashrc; export PATH=$HOME/bin:$PATH; hash -r; pnpm --version" > "$HOME/pnpm-verify.log" 2>&1 || {
...
if [ ! -d "$HOME/Roo-Code" ]; then
  bash -l -c "source $HOME/.bashrc; export PATH=$HOME/bin:$PATH; hash -r; git clone https://github.com/StevenTCramer/Roo-Code.git $HOME/Roo-Code" > "$HOME/git-clone.log" 2>&1 || {
...
bash -l -c "source $HOME/.bashrc; export PATH=$HOME/bin:$PATH; hash -r; pnpm install" > "$HOME/pnpm-install.log" 2>&1 || {
...
bash -l -c "source $HOME/.bashrc; export PATH=$HOME/bin:$PATH; hash -r; pnpm run setup" > "$HOME/pnpm-setup-run.log" 2>&1 || {
